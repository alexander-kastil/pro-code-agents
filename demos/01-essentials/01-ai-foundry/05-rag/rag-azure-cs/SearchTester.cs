...
public class SearchTester
{
    ...
    private readonly object _embeddingsClient;
    private readonly AzureConfiguration _config;

    public SearchTester(SearchClient searchClient, object embeddingsClient, AzureConfiguration config)
    {
    ...
    private async Task<List<Azure.Search.Documents.Models.SearchDocument>> VectorSearchAsync(string query, int topK = 3)
    {
        try
        {
            ...
            var searchResult = await _searchClient.SearchAsync<Azure.Search.Documents.Models.SearchDocument>(null, searchOptions);
            
            var results = new List<Azure.Search.Documents.Models.SearchDocument>();
        ...
        catch (Exception ex)
        {
            ...
            return new List<Azure.Search.Documents.Models.SearchDocument>();
        }
    }

    private async Task<float[]> GenerateEmbeddingAsync(string text)
    {
        if (_embeddingsClient is AzureOpenAIClient openAIClient)
        {
        ...
        else if (_embeddingsClient is EmbeddingsClient inferenceClient)
        {
