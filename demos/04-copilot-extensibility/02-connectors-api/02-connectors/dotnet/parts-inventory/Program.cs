...
{
    var connectionId = PromptForConnectionId();
...
static string PromptForConnectionId()
{
    const string pattern = "^[a-z][a-z0-9]{2,47}$";

    while (true)
    {
        var input = PromptForInput(
            "Enter a unique ID for the new connection (3-48 lowercase letters or numbers, must start with a letter)",
            true)?.Trim().ToLowerInvariant();

        if (input is not null && Regex.IsMatch(input, pattern, RegexOptions.CultureInvariant))
        {
            return input;
        }

        Console.WriteLine("Invalid connection ID. Please use 3-48 characters, start with a letter, and use only lowercase letters and numbers.");
    }
}